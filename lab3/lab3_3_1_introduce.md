...
**ecall**：这是主动发起trap的指令。它的行为高度依赖于执行它的当前特权级。在 U 模式下执行ecall，会触发一个 ecall-from-m-mode-exception，这是应用程序请求操作系统服务的标准方式，路径为 **U → S**。在S模式下执行ecall，则会触发 ecall-from-s-mode-exception，这通常用于内核向M模式固件（如OpenSBI）请求服务，路径为 **S → M**。它本质上是提升特权级的工具。
