...
在这次实验中，我们创建了一个名为 RISCV 的环境变量，它用于指定你安装 RISC-V 相关软件的位置。这是为了让系统能够知道 RISC-V 工具链（比如编译器、调试工具等）在哪里，从而可以在终端中方便地执行相关命令。

比如你在终端输入 `$RISCV/bin/riscv64-unknown-elf-gcc` 时，操作系统就知道 RISCV 指定的路径在哪，它会根据这个路径找到编译工具，避免你每次都要手动输入完整路径。
