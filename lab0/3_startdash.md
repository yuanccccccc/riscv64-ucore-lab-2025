...
>
> 为什么需要环境变量？环境变量让我们可以在命令行中使用方便的别名或快捷方式，而不必每次都输入完整的路径。例如，系统会根据 PATH 环境变量来查找程序的位置。当你在终端输入一个命令时，操作系统会根据 PATH 中列出的路径去查找对应的可执行文件。
>
> 在这次实验中，我们创建了一个名为 RISCV 的环境变量，它用于指定你安装 RISC-V 相关软件的位置。这是为了让系统能够知道 RISC-V 工具链（比如编译器、调试工具等）在哪里，从而可以在终端中方便地执行相关命令。比如你在终端输入 `$RISCV/bin/riscv64-unknown-elf-gcc` 时，操作系统就知道 RISCV 指定的路径在哪，它会根据这个路径找到编译工具，避免你每次都要手动输入完整路径。
>
...
**Ubuntu虚拟机**中，可以使用较为简单的图形化的gedit作为编辑器，（gedit使用可以参考博客：https://blog.csdn.net/thisway_diy/article/details/108728087 ）。
