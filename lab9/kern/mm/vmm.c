...
int do_mmap(struct mm_struct *mm, uintptr_t addr, size_t len, uint32_t vm_flags,
            struct file *file, off_t offset)
{
    ...
    if (addr == 0) {
        // 从0x60000000开始查找空闲区域（用户空间的mmap区域）
        uintptr_t search_addr = 0x60000000;
        uintptr_t search_end = USERTOP - len;
        
        while (search_addr < search_end) {
            struct vma_struct *vma = find_vma(mm, search_addr);
            if (vma == NULL) {
                // 没有找到包含此地址的vma，检查是否有足够空间
                struct vma_struct *next_vma = NULL;
                list_entry_t *list = &(mm->mmap_list), *le = list;
                while ((le = list_next(le)) != list) {
                    struct vma_struct *v = le2vma(le, list_link);
                    if (v->vm_start > search_addr) {
                        next_vma = v;
                        break;
                    }
                }
                if (next_vma == NULL || next_vma->vm_start >= search_addr + len) {
                    addr = search_addr;
                    break;
                }
                search_addr = next_vma->vm_end;
            } else {
                // 当前地址在某个vma中，跳到该vma的末尾
    ...
    {
        vma->vm_file = file;
        vma->vm_pgoff = (offset > 0) ? (offset / PGSIZE) : 0;
...
int do_munmap(struct mm_struct *mm, uintptr_t addr, size_t len)
{
    ...
    struct vma_struct *vma = find_vma(mm, addr);
    if (vma == NULL || vma->vm_start != addr || vma->vm_end != addr + len)
    {
        return -E_INVAL;
    }
...
// // mmap_pgfault - handle page fault for file-backed mapping
// int mmap_pgfault(struct mm_struct *mm, uint32_t error_code, uintptr_t addr)
// {
//     struct vma_struct *vma = find_vma(mm, addr);
//     if (vma == NULL || vma->vm_start > addr)
//     {
...
//     // Load data from file if file-backed
//     if (vma->vm_file != NULL)
//     {
//         size_t page_offset = (addr - vma->vm_start) / PGSIZE + vma->vm_pgoff;
//         off_t file_offset = page_offset * PGSIZE;

//         size_t read_size = 0;
//         int ret = file_read(vma->vm_file->fd, kva, PGSIZE, &read_size);
//         if (ret != 0)
//         {
//             return ret;
//         }
